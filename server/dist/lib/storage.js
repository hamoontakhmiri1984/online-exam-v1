...
exports.uploadObjectFromFile = uploadObjectFromFile;
exports.getSignedDownloadUrl = getSignedDownloadUrl;
exports.deleteObject = deleteObject;
exports.uploadPublicObject = uploadPublicObject;
exports.getPublicObjectUrl = getPublicObjectUrl;
exports.deletePublicObject = deletePublicObject;
...
async function uploadObjectFromFile(key, filePath, sizeBytes, contentType) {
    await s3.send(new client_s3_1.PutObjectCommand({
        Bucket: env_1.env.S3_BUCKET,
        Key: key,
...
async function getSignedDownloadUrl(key, expiresInSeconds = env_1.env.SIGNED_URL_EXPIRES_IN) {
    const command = new client_s3_1.GetObjectCommand({ Bucket: env_1.env.S3_BUCKET, Key: key });
...
async function deleteObject(key) {
    await s3.send(new client_s3_1.DeleteObjectCommand({ Bucket: env_1.env.S3_BUCKET, Key: key }));
}
// ---------------------------------------------------------------------------
...
async function uploadPublicObject(key, body, contentType) {
    if (!env_1.env.S3_PUBLIC_BASE_URL) {
        throw new Error('S3_PUBLIC_BASE_URL تنظیم نشده - قبل از آپلود تصویر بلاگ/CMS باید یه bucket/CDN عمومی کانفیگ بشه');
    }
    await s3.send(new client_s3_1.PutObjectCommand({
        Bucket: env_1.env.S3_BUCKET,
        Key: `public/${key}`,
...
function getPublicObjectUrl(key) {
    return `${env_1.env.S3_PUBLIC_BASE_URL.replace(/\/$/, '')}/public/${key}`;
}
async function deletePublicObject(key) {
    await s3.send(new client_s3_1.DeleteObjectCommand({ Bucket: env_1.env.S3_BUCKET, Key: `public/${key}` }));
}
