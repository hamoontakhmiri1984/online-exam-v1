...
exports.registerIpFailure = registerIpFailure;
exports.clearIpFailures = clearIpFailures;
...
const failKey = (userId) => `login:fail:${userId}`;
const lockKey = (userId) => `login:lock:${userId}`;
async function isAccountLocked(userId) {
    const locked = await redis_1.redis.get(lockKey(userId));
    return locked !== null;
}
...
async function registerFailedAttempt(userId) {
    ...
    if (count === 1) {
        ...
        await redis_1.redis.expire(key, (0, duration_1.durationToSeconds)(FAILED_ATTEMPTS_WINDOW));
    }
    if (count >= exports.MAX_FAILED_ATTEMPTS) {
        await redis_1.redis.set(lockKey(userId), '1', {
            EX: (0, duration_1.durationToSeconds)(LOCKOUT_DURATION),
        });
    }
    return count;
}
...
async function clearFailedAttempts(userId) {
    await redis_1.redis.del(failKey(userId));
    await redis_1.redis.del(lockKey(userId));
}
...
const ipFailKey = (ip) => `login:ipfail:${ip}`;
const ipCaptchaKey = (ip) => `login:ipcaptcha:${ip}`;
async function isCaptchaRequired(ip) {
    const flagged = await redis_1.redis.get(ipCaptchaKey(ip));
    return flagged !== null;
}
// یه تلاش ناموفق (پسورد غلط، حساب پیدا نشد، کپچای غلط) رو برای این IP ثبت
// می‌کنه؛ بعد از رسیدن به سقف، کپچا رو براش فعال می‌کنه
async function registerIpFailure(ip) {
    const key = ipFailKey(ip);
    const count = await redis_1.redis.incr(key);
    if (count === 1) {
        await redis_1.redis.expire(key, (0, duration_1.durationToSeconds)(IP_FAIL_WINDOW));
    }
    if (count >= IP_FAIL_TRIGGER) {
        await redis_1.redis.set(ipCaptchaKey(ip), '1', {
            EX: (0, duration_1.durationToSeconds)(IP_CAPTCHA_DURATION),
        });
    }
}
...
async function clearIpFailures(ip) {
    await redis_1.redis.del(ipFailKey(ip));
}
